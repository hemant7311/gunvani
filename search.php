...
$articles = [];
try {
    $stmt = $pdo->prepare("SELECT a.*, c.name AS category_name FROM articles a LEFT JOIN categories c ON a.category_id = c.id WHERE a.status = 'published' AND (a.title LIKE ? OR a.summary LIKE ? OR a.content LIKE ? OR c.name LIKE ?) ORDER BY a.id DESC LIMIT 50");
